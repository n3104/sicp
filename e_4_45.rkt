...
(define (amb-collect exp)
  (define (amb-collect-loop)
    (let ((result (try-again))) 
      (if (eq? result 'failed)
          nil
          (cons result (amb-collect-loop)))))
  (cons (eval exp) (amb-collect-loop)))
...
; https://sicp.iijlab.net/fulltext/x231.html#index1077
(eval
 '(define (memq item x)
    (cond ((null? x) false)
          ((eq? item (car x)) x)
...
; https://sicp.iijlab.net/fulltext/x432.html
(eval '(define nouns '(noun student professor cat class)))
(eval '(define verbs '(verb studies lectures eats sleeps)))
(eval '(define articles '(article the a)))
(eval '(define *unparsed* '()))
(eval '(define prepositions '(prep for to in by with)))
...
(check-equal? (length (amb-collect '(parse '(the professor lectures to the student in the class with the cat)))) 5)
(amb-collect '(parse '(The professor lectures to the student in the class with the cat)))
