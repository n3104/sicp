...
(define (analyze-ramb exp)
  (let ((cprocs (map analyze (amb-choices exp))))
    (let ((random-cprocs (randomize-list cprocs)))
      (lambda (env succeed fail)
        (define (try-next choices)
          (if (null? choices)
              (fail)
...
(define (randomize-list items)
  (define (randomize-list-iter items)
    (if (null? items)
        nil
        (let ((item-length (length items)))
          (if (= item-length 1)
              items
              (let ((item (list-ref items (random item-length))))
                (cons item (randomize-list-iter (remove item items))))))))
  (randomize-list-iter items))
...
; 追加実装箇所
(amb-collect '(list (ramb 1 2 3 4 5)))
