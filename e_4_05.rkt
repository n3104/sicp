...
; 以下、追加実装
(define (cond-has-recipient? exp) (tagged-list? exp '=>))

(define (cond-recipient exp) (cadr exp))
      ...
      (let ((first (car clauses))
        ...
        (if (cond-else-clause? first)
            ...
            (make-if (cond-predicate first)
                     ; cond-actions の先頭が => の場合の処理を追加した。
                     (let ((cond-action-clauses (cond-actions first)))
                       (if (cond-has-recipient? cond-action-clauses)
                           (list (cond-recipient cond-action-clauses) (cond-predicate first))
                           (sequence->exp cond-action-clauses)))
                     (expand-clauses rest))))))
...
; 追加実装箇所
(check-equal? (eval '(cond ((assoc 'b '((a 1) (b 2))) => cadr)(else false)) the-global-environment) 2)
(check-equal? (eval '(cond ((eq? 1 2) true)((assoc 'a '((a 1) (b 2))) => cadr)(else false)) the-global-environment) 1)
(display 'ok)