...
; 追加実装箇所
; https://wizardbook.wordpress.com/2011/01/22/exercise-4-52/
(define (if-fail? exp)
  (tagged-list? exp 'if-fail))

(define if-fail-success cadr)
...
(check-equal? 
 (eval '(if (let ((x (an-element-of '(1 3 5))))
           (require (even? x))
           x)
         'all-odd))
 'failed)
...
(check-equal? 
 (eval '(if-fail (let ((x (an-element-of '(1 3 5))))
           (require (even? x))
           x)
         'all-odd))
 'all-odd)
...
(check-equal? 
 (eval '(if-fail (let ((x (an-element-of '(1 3 5 8))))
           (require (even? x))
           x)
         'all-odd))
 8)
