...
(define (eval exp env)
  (cond ((self-evaluating? exp) exp)
        ...
        ((syntax? exp) (eval-syntax exp env))        
        ((application? exp)
...
(define (put-syntax name function)
  (put 'syntax name function))

(define (get-syntax name)
  (get 'syntax name))

(define (syntax? exp)
  (if (eq? (get-syntax (car exp)) false)
      false
      true))

(define (eval-syntax exp env)
  ((get-syntax (car exp)) exp env))

(put-syntax 'define eval-definition)
(put-syntax 'quote (lambda (exp env)
