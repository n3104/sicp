...
; 以下、追加実装
(define (abstract-scan vars vals var vars-null-func var-eq-vars-func)
  (cond ((null? vars)
         (vars-null-func))
        ((eq? var (car vars))
         (var-eq-vars-func vals))
        (else (abstract-scan (cdr vars) (cdr vals) var vars-null-func var-eq-vars-func))))

(define (lookup-variable-value var env)
  (define (env-loop env)
    (define (scan vars vals)
      (abstract-scan vars vals var
...
; setup-environment 内部で define-variable! を利用するため実行タイミングをここにした。
