...
(check-equal? (eval '(call + x 2) the-global-environment) 5)
(check-equal? (eval '(quote x) the-global-environment) 'x)
(check-equal? (eval '(set! x 5) the-global-environment) 'eval-assignment)
(check-equal? (eval '(call + x 2) the-global-environment) 7)
(check-equal? (eval '(if true x) the-global-environment) 5)
(check-equal? (eval '(begin (call + x 2)(call - x 2)) the-global-environment) 3)
(check-equal? (eval '(call (lambda (y) (call + y y)) x) the-global-environment) 10)
(check-equal? (eval '(cond ((call eq? x 5) x)(else false)) the-global-environment) 5)
(check-equal? (eval '(cond ((call eq? x 4) x)(else false)) the-global-environment) false)
(display 'ok)