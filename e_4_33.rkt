...
; https://sicp.iijlab.net/fulltext/x423.html
...
(define (eval exp env)
  (cond ((self-evaluating? exp) exp)
        ...
        ((quoted? exp) (eval-quoted exp env)) ; eval-quoted に変更している。
...
; https://wizardbook.wordpress.com/2011/01/09/exercise-4-33/
...
(define (text-of-quotation exp env)
  (let ((result (cadr exp)))
    (if (pair? result)
        (eval (make-list result) env) ; ここで eval するので引数に env が必要になる。
        result)))
...
; 追加実装箇所
(check-equal? (actual-value '(car '(a b c)) the-global-environment) 'a)
