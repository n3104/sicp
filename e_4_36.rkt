...
(eval
 '(define (a-pythagorean-triple-from low)
    ...
    (define (sqrt x)
      (define (sqrt-iter guess x)
        (define (good-enough? guess x)
          (< (abs (- (square guess) x)) 0.0000000001)) ; この精度にしないと (3 4 5) を取得できなかった。
    ...
    (let ((i (an-integer-starting-from low)))
      (let ((j (an-integer-between i (limit i)))) ; j は i の値から一定の範囲を limit 関数で算出するようにしている。
        (let ((k (sqrt (+ (square i) (square j))))) ; k は amb にせず i と j から算出するようにしている。
          (require (integer? k))
          (list i j k)))))
 )

(check-equal? (eval '(a-pythagorean-triple-from 1)) '(3 4 5.0))
(check-equal? (try-again) '(5 12 13.0))
(check-equal? (try-again) '(6 8 10.0))
