...
; 以下、追加実装
; https://sicp.iijlab.net/fulltext/x221.html#index879
; 名前が衝突しないように map-eval とした。また、必要な関数および定義をそれぞれ setup-environment と primitive-procedures に追加した。
(check-equal?
 (eval '
       (define (map-eval proc items)
         (if (null? items)
             nil
             (cons (proc (car items))
                   (map-eval proc (cdr items)))))
       the-global-environment)
 'eval-definition)
(check-equal? (eval '(map-eval (lambda (x) (* x x))(list 1 2 3 4)) the-global-environment) (list 1 4 9 16))

; 基本手続きの map の場合自作の eval を経由せずに内部でそのまま lambda を実行しようとする。その結果、'procedure というタグを実行しようとしてエラーになる模様。
(#%require (only racket/base with-handlers))
(#%require (only racket/base exn:fail?))
(#%require (only racket/base displayln))
(#%require (only racket/base exn-message))
(with-handlers ([exn:fail? (lambda (exn) (displayln (exn-message exn)))])
  (eval '(map (lambda (x) (* x x))(list 1 2 3 4)) the-global-environment))
