...
(check-equal? (eval '(define y 4) the-global-environment) 'eval-definition)
(check-equal? (eval '(+ y 2) the-global-environment) 6)
(check-equal? (eval '(unbind! y) the-global-environment) 'eval-unbind)
; unbind! 済みなので Unbound variable define というエラーになる。
(#%require (only racket/base with-handlers))
(#%require (only racket/base exn:fail?))
(#%require (only racket/exn exn->string))
(with-handlers ([exn:fail? (lambda (exn) (check-equal? (exn->string exn) "Unbound variable y\n"))])
  (eval '(+ y 2) the-global-environment))
(display 'ok)