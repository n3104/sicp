...
(define (eval-and exps env)
  (cond ((last-exp? exps) (eval (first-exp exps) env))
        (else (if (eval (first-exp exps) env)
                  (eval-and (rest-exps exps) env) ; true の場合は次のactionを評価する。
                  false))))
...
; 追加実装箇所
(check-equal? (eval '(and (eq? 1 1)(eq? 1 1)) the-global-environment) true)
(check-equal? (eval '(and (eq? 1 2)(eq? 1 1)) the-global-environment) false)
(check-equal? (eval '(and (eq? 1 1)(eq? 1 2)) the-global-environment) false)
(check-equal? (eval '(and (eq? 1 2)(eq? 1 2)) the-global-environment) false)
(check-equal? (eval '(or (eq? 1 1)(eq? 1 1)) the-global-environment) true)
(check-equal? (eval '(or (eq? 1 2)(eq? 1 1)) the-global-environment) true)
(check-equal? (eval '(or (eq? 1 1)(eq? 1 2)) the-global-environment) true)
(check-equal? (eval '(or (eq? 1 2)(eq? 1 2)) the-global-environment) false)
